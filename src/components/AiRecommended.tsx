import { Button, Input, Box, Heading, Spinner, Image, Highlight, Text } from '@chakra-ui/react';
...
const AiRecommended = () => {
  ...
  return (
    <>
      <Box>
        <Heading size="md" mb={4} color='purple.500'>Get a random Movie or Tv Series</Heading>
        <AiInput isFetchingMovie={isFetchingMovie} />

        <Box display="flex" alignItems="center">
          {(aiError || isError) &&
            <Highlight query={["Failed to respond please try again"]} styles={{ color: 'red.600' }}>
...
const AiInput = ({ isFetchingMovie }: { isFetchingMovie: boolean }) => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className='flex gap-2'>
      <Input mb={2} placeholder="Describe a movie, genre, or mood..." {...register('query', { required: true })} />

      {/* Search Button */}
      <Button variant='subtle' color="purple.500" type='submit' disabled={isFetchingAi || isFetchingMovie}>
        <BiSearch />
      </Button>

      {/* Random Response Generate Button */}
      <Button variant='subtle' loadingText={<>Loading <BsStars /></>} color="purple.500" onClick={handleClick} loading={isFetchingAi || isFetchingMovie}>
        <RiAiGenerate2 />
        Random
      </Button>
    </form>
