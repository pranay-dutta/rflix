...
import { Blockquote, Box, IconButton, Image, Progress, Text } from "@chakra-ui/react";
...
const RectOverlayCard = ({ media }: Props) => {
  const mediaType = isMovie(media) ? "movie" : "tv";
  const activePalette = useCustomizationStore((s) => s.activePalette);

  const [isPreviewActive, setIsPreviewActive] = useState(false);

  const { data: posterAndExternalIds, isLoading: posterLoading } = useRectPoster(
    media.id,
    mediaType,
  );
  const imdbId = posterAndExternalIds?.external_ids.imdb_id;
  const posterPath = getPosterURL(posterLoading, posterAndExternalIds);
  ...
  return (
    <Skeleton loading={posterLoading} overflow="hidden" borderRadius="sm">
      <Box
        position="relative"
        aspectRatio={16 / 9}
        cursor="pointer"
...
export default RectOverlayCard;
